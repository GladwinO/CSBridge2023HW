...
int main()
{
    ...
    LList<BarAttendee> attendList;
    while (userFileName >> person) {//I may need to use a getline here instead of >> to get the whole line
        if (attendList.isEmpty()) {
            attendList.insertAtHead(person);
        }
        attendList.insertAtEnd(person);
    }
    int listSize = attendList.size();

    for (int i = 0; i < listSize - 1; i++) {
        BarAttendee curHeadData;
        curHeadData = attendList.removeFromHead();
        std::cout << curHeadData.getAmountOwed() << /*"\t" << curHeadData.getUserFullName() <<*/ std::endl;
...
std::istream& operator>>(std::istream& ins, BarAttendee& person) {
    ...
    //how will it work/end when a file is inputed instead?
